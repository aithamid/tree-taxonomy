export let data: object[] = [
{
    'Id': 'l1-1',
    'Name': 'Physical Infrastructure',
    'Designation': '',
    'IsExpand': 'true',
    'RatingColor': '#C34444'
},
{
    'Id': 'l1-2',
    'Name': 'Scenery',
    'Designation': '',
    'IsExpand': 'true',
    'RatingColor': '#C34444'
},
{
    'Id': 'l1-3',
    'Name': 'Environmental conditions',
    'Designation': '',
    'IsExpand': 'true',
    'RatingColor': '#C34444'
},
{
    'Id': 'l1-4',
    'Name': 'Traffic conditions',
    'Designation': '',
    'IsExpand': 'true',
    'RatingColor': '#C34444'
},
{
    'Id': 'l1-5',
    'Name': 'Digital infrastructure',
    'Designation': '',
    'IsExpand': 'true',
    'RatingColor': '#C34444'
},
{
    'Id': 'l1-6',
    'Name': 'Vehicle Features',
    'Designation': '',
    'IsExpand': 'true',
    'RatingColor': '#C34444'
},
{
    'Id': 'l2-1',
    'Name': 'Roadway Type',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l2-2',
    'Name': 'Roadway Edge',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l2-3',
    'Name': 'Roadway geometry',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l2-4',
    'Name': 'Junctions',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l2-5',
    'Name': 'Temporary structures',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l2-6',
    'Name': 'Fixed surrounding structures',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l2-7',
    'Name': 'Special structures',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l2-8',
    'Name': 'Signage',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l2-9',
    'Name': 'Man-made landmark',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-1'
},
{
    'Id': 'l3-1',
    'Name': 'Road category',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-2',
    'Name': 'Specific infrastructure configuration',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-3',
    'Name': 'Roadway general orientation',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-4',
    'Name': 'Eventual roadway use type',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-5',
    'Name': 'Ego allowed to drive on traffic lane n°i',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-6',
    'Name': 'Use of lanes',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l4-1',
    'Name': 'Use of the ego lane',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-6'
},
{
    'Id': 'l4-2',
    'Name': 'Use of the left lane',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-6'
},
{
    'Id': 'l4-3',
    'Name': 'Use of the right lane',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-6'
},
{
    'Id': 'l4-4',
    'Name': 'Use of traffic lane n°i',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-6'
},
{
    'Id': 'l3-7',
    'Name': 'Traffic lanes direction',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-8',
    'Name': 'Element on the lanes not preventing traffic',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l4-5',
    'Name': 'Possible element on the ego lane not preventing traffic',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-8'
},
{
    'Id': 'l4-6',
    'Name': 'Element on the traffic lane n°i not preventing traffic',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-8'
},

{
    'Id': 'l3-9',
    'Name': 'type of pavement surface',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-10',
    'Name': 'Luminance of the road surface',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-11',
    'Name': 'Pavement grip coefficient',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-12',
    'Name': 'Road marking contrast',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-1'
},
{
    'Id': 'l3-13',
    'Name': 'Element of Infrastructure adjacent to the lane',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-2'
},
{
    'Id': 'l4-7',
    'Name': 'Nature of the element of infrastructure\n adjacent to ego lane on the left side',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-13'
},
{
    'Id': 'l4-8',
    'Name': 'Nature of the element of infrastructure\n adjacent to ego lane on the right side',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-13'
},
{
    'Id': 'l4-9',
    'Name': 'Element of Infrastructure adjacent\n to the lane 1 on its left side',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-13'
},
{
    'Id': 'l4-10',
    'Name': 'Element of Infrastructure  adjacent\n to the lane i on its right side',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-13'
},
{
    'Id': 'l3-14',
    'Name': 'Width of the lanes',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-3'
},
{
    'Id': 'l4-11',
    'Name': 'Minimal width of the ego lane',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-14'
},
{
    'Id': 'l4-12',
    'Name': 'Width of the lane n° i',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-14'
},
{
    'Id': 'l3-15',
    'Name': 'Cross slope',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-3'
},
{
    'Id': 'l3-16',
    'Name': 'Radius of curvature',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-3'
},
{
    'Id': 'l3-17',
    'Name': 'Length of the link section',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-3'
},
{
    'Id': 'l3-18',
    'Name': 'Slope',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-3'
},
{
    'Id': 'l3-19',
    'Name': 'Unsuitable area for stopping the vehicle',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-3'
},
{
    'Id': 'l3-20',
    'Name': 'Configuration of the intersection',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-4'
},
{
    'Id': 'l3-21',
    'Name': 'Number of branches of the intersection branch',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-4'
},
{
    'Id': 'l3-22',
    'Name': 'Angle of the crossing lanes',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-4'
},
{
    'Id': 'l3-23',
    'Name': 'Angle of the crossing lanes',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-4'
},
{
    'Id': 'l3-24',
    'Name': 'Priority rules',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-4'
},
{
    'Id': 'l3-25',
    'Name': 'Particularity of the intersection',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-4'
},
{
    'Id': 'l3-26',
    'Name': 'Workzone',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-5'
},
{
    'Id': 'l3-27',
    'Name': 'FLR',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-5'
},
{
    'Id': 'l3-28',
    'Name': 'Constraints on masks from the visibility point of view',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-6'
},
{
    'Id': 'l3-29',
    'Name': 'Constraints on masks from the geo-positioning point of view',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-6'
},
{
    'Id': 'l3-30',
    'Name': 'Crossing of vulnerable users',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-7'
},
{
    'Id': 'l3-31',
    'Name': 'Marking of cycle zone on the ego lane',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-7'
},
{
    'Id': 'l3-32',
    'Name': 'Marking of cycle zone on the ego lane',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-7'
},
{
    'Id': 'l3-33',
    'Name': 'Vertical traffic signs',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l4-13',
    'Name': 'Type A : hazard',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l4-14',
    'Name': 'Type AB : priorities at intersection ',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l4-15',
    'Name': 'Type B prescriptions',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l4-16',
    'Name': 'Type C useful indications',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l4-17',
    'Name': 'Type CE useful services indication',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l4-18',
    'Name': 'Type D signalization and positioning',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l4-19',
    'Name': 'Type E entries/end of urban zones',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l4-20',
    'Name': 'Type G Railway crossing localization',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l4-21',
    'Name': 'Type SR information for road safety',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-33'
},
{
    'Id': 'l3-34',
    'Name': 'Traffic lights',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l3-35',
    'Name': 'Road markings (horizontal markings)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l3-36',
    'Name': 'Guidance equipment',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l3-37',
    'Name': 'Boundary markers',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l3-38',
    'Name': 'Closing devices',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l3-39',
    'Name': 'Dynamic signs ',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l3-40',
    'Name': 'Temporary signs',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l4-22',
    'Name': 'Type AK : hazard',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-40'
},
{
    'Id': 'l4-23',
    'Name': 'Type K : Specific',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-40'
},
{
    'Id': 'l4-24',
    'Name': 'Type KC : Workzones',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-40'
},
{
    'Id': 'l4-25',
    'Name': 'Type KD : Lane merging',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-40'
},
{
    'Id': 'l3-41',
    'Name': 'Connected equipments',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-8'
},
{
    'Id': 'l3-42',
    'Name': 'Infrared objects',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-9'
},
{
    'Id': 'l3-43',
    'Name': 'Beacon',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-9'
},
{
    'Id': 'l3-44',
    'Name': 'Speed bump',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-9'
},
{
    'Id': 'l3-45',
    'Name': 'Electromagnetic',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-9'
},
{
    'Id': 'l2-55',
    'Name': 'Specific zones',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-2'
},
{
    'Id': 'l2-56',
    'Name': 'Region/State',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-2'
},
{
    'Id': 'l2-57',
    'Name': 'Interference zones',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-2'
},
{
    'Id': 'l2-58',
    'Name': 'Geo-fencing',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-2'
},
{
    'Id': 'l2-59',
    'Name': 'Landmark',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-2'
},
{
    'Id': 'l2-60',
    'Name': 'Weather conditions',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-3'
},
{
    'Id': 'l3-46',
    'Name': 'Rain',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-60'
},
{
    'Id': 'l3-47',
    'Name': 'Snow',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-60'
},
{
    'Id': 'l3-48',
    'Name': 'Hall',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-60'
},
{
    'Id': 'l3-49',
    'Name': 'Fog',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-60'
},
{
    'Id': 'l3-49',
    'Name': 'Wind',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-60'
},
{
    'Id': 'l2-61',
    'Name': 'Particulates',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-3'
},
{
    'Id': 'l2-62',
    'Name': 'Weather-induced roadway conditions',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-3'
},
{
    'Id': 'l2-63',
    'Name': 'Illumination',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-3'
},
{
    'Id': 'l3-50',
    'Name': 'Illumination level',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-63'
},
{
    'Id': 'l3-51',
    'Name': 'Illumination conditions',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-63'
},
{
    'Id': 'l3-52',
    'Name': 'Illumination illuminances',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-63'
},
{
    'Id': 'l3-53',
    'Name': 'Illumination variations',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-63'
},
{
    'Id': 'l2-64',
    'Name': 'Minimum/Maximum ambient air temperature',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-3'
},
{
    'Id': 'l2-65',
    'Name': 'Traffic density',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-4'
},
{
    'Id': 'l3-54',
    'Name': 'Traffic density on the ego direction traffic lane(s)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-65'
},
{
    'Id': 'l3-55',
    'Name': 'Traffic density on the opposite direction traffic lane(s) ',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-65'
},
{
    'Id': 'l3-56',
    'Name': 'Traffic density on ramp in',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-65'
},
{
    'Id': 'l3-57',
    'Name': 'Traffic density on ramp off',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-65'
},
{
    'Id': 'l3-58',
    'Name': 'Traffic density on the crossing lane(s)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-65'
},
{
    'Id': 'l3-59',
    'Name': 'Traffic density on the traffic lane(s) n°i',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-65'
},
{
    'Id': 'l2-66',
    'Name': 'Road-users (type & speed)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-4'
},
{
    'Id': 'l3-60',
    'Name': 'Road-users type',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-66'
},
{
    'Id': 'l4-26',
    'Name': 'Road-users type on the ego direction traffic lane(s)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-60'
},
{
    'Id': 'l4-27',
    'Name': 'Road-users type on the opposite direction traffic lane(s)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-60'
},
{
    'Id': 'l4-28',
    'Name': 'Road-users type on the crossing traffic lane(s)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-60'
},
{
    'Id': 'l4-29',
    'Name': 'Road-users type on the traffic lane n°i',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-60'
},
{
    'Id': 'l3-61',
    'Name': 'Road-users speed',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-66'
},
{
    'Id': 'l4-30',
    'Name': 'Road users speed on the ego direction traffic lane(s)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-61'
},
{
    'Id': 'l4-31',
    'Name': 'Road users speed on the opposite direction traffic lane(s)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-61'
},
{
    'Id': 'l4-32',
    'Name': 'Road users speed on the crossing traffic lane(s)',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-61'
},
{
    'Id': 'l4-33',
    'Name': 'Road users speed on the traffic lane n° i',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l3-61'
},
{
    'Id': 'l2-67',
    'Name': 'Road users behavior',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l1-4'
},
{
    'Id': 'l3-62',
    'Name': 'Risky behavior of third parties',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-67'
},
{
    'Id': 'l3-63',
    'Name': 'Particularly accident-prone areas',
    'Designation': '',
    'IsExpand': 'false',
    'RatingColor': '#68C2DE',
    'ReportingPerson': 'l2-67'
},
];